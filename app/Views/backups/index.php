            ...
            <table class="table table-hover align-middle mb-0">
                    ...
                    <tr>
                        <td colspan="7" class="text-center py-5">
                            ...
                            <h6 class="text-muted fw-normal">
                                No backup logs found. Your safety list is empty.
                            </h6>

                            <a href="<?= base_url('backups/add') ?>" class="btn btn-sm btn-link text-indigo mt-2">
                ...
                <?php else: ?>
                    ...
                    <?php foreach($backups as $b):
                        ...
                        $shortNote = strlen($note) > 80 ? substr($note, 0, 80) . '...' : $note;
                    ?>
                    ...
                    <tr>
                        ...
                        <td class="ps-4 py-3">
                            <div class="d-flex align-items-center">
                                ...
                                <div>
                                    ...
                                    <?php if(!empty($b['domain_name'])): ?>
                                        <small class="text-primary d-block fw-semibold">
                                            <?= esc($b['order_type'] ?? 'Order') ?>: <?= esc($b['domain_name']) ?>
                                        </small>
                                    <?php else: ?>
                        ...
                        <td>
                            <?php if(!empty($note)): ?>
                                <div class="desc-preview">
                                    ...
                                    <?php if(strlen($note) > 80): ?>
                                        <button type="button"
                                                class="read-more-btn"
                                                onclick='viewDescription(<?= json_encode($b['client_name'], JSON_HEX_APOS | JSON_HEX_QUOT) ?>, <?= json_encode($note, JSON_HEX_APOS | JSON_HEX_QUOT) ?>)'>
                                            View More
                                        </button>
                                    <?php endif; ?>
                                </div>
                        ...
                        <td>
                            <div class="text-dark small fw-medium">
                                <?= date('d M, Y', strtotime($b['last_backup_date'])) ?>
                            </div>

                            <div class="extra-small text-muted" style="font-size: 10px;">
                                Recorded
                            </div>
                        </td>
                        ...
                        <td>
                            <span class="badge bg-light text-dark border fw-normal py-2 px-3 rounded-3">
                                <i class="fas fa-calendar-alt me-1 text-indigo"></i>
                                <?= $b['backup_interval'] ?> Mo
                            </span>
                        </td>
                        ...
                        <td>
                            ...
                            <?php else: ?>
                                <span class="badge rounded-pill text-success px-3 py-2 d-inline-flex align-items-center" style="background: rgba(16, 185, 129, 0.1);">
                                    <span class="status-dot dot-uptodate"></span>
                                    Safe
                                </span>
                            <?php endif; ?>
                        </td>
                        ...
                        <td class="text-end pe-4">
                            <div class="d-flex justify-content-end gap-2">
                                ...
                                <button
                                    onclick="confirmTask('<?= base_url('backups/mark-done/'.$b['id']) ?>', 'Mark this client as backed up today?')"
                                ...
                                <a
                                    href="<?= base_url('backups/edit/'.$b['id']) ?>"
                                ...
                                <button
                                    onclick="confirmDelete('<?= base_url('backups/delete/'.$b['id']) ?>')"
...
<script>
    function viewDescription(clientName, description) {
        Swal.fire({
            title: 'Backup Description',
            html: '<div style="text-align:left;line-height:1.6;font-size:14px;color:#374151;">'
